    ...
    <Tabs defaultActiveKey="buy" className="bg-dark text-white">
      <Tab eventKey="buy" title="Buy" className="bg-dark">
        <form
          onSubmit={(event) => {
            ...
            makeBuyOrder(dispatch, bar, token, web3, buyOrder, account);
          }}
        >
          ...
          {showBuyTotal ? (
            <small>Total: {buyOrder.price * buyOrder.amount} ETH</small>
          ) : null}
        </form>
      </Tab>
      <Tab eventKey="sell" title="Sell" className="bg-dark">
        <form
          onSubmit={(event) => {
            event.preventDefault();
            makeSellOrder(dispatch, bar, token, web3, sellOrder, account);
          }}
        >
          <div className="form-group small">
    ...
    return (
      <div className="card bg-dark text-white">
        ...
        <div className="card-body">
          {this.props.showForm ? showForm(this.props) : <Spinner />}</div>
      </div>
    );
  }
}
